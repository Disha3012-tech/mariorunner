...
function draw() {
  ...
  drawSprites();
  if (gameState===PLAY){
    //for incrementing the score by 1
    if(coinGroup.isTouching(mario)){
      //for playing the sound when mario touches the coin
      sound.play();
      score=score+1;
      //to destroy the thecoin when mario touches the coin
      coinGroup[0].destroy();
    }
   ...
   if(obstaclesGroup.isTouching(mario)){
    ...
    if(mousePressedOver(restart)) {
      // to make mario's life to positive number
      if(life>0){
        reset();
      }
